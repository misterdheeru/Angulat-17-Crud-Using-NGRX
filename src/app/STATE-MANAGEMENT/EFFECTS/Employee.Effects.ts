...
export class EmployeeEffect {
  ...
  FetchEmployees$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(() =>
        this.service
          .getProducts()
          .pipe(map((res) => setemployees({ STUDENTS: res })))
      )
    )
  );

  AddStudentRecord = createEffect(() =>
    this.actions$.pipe(
      ofType(addstudent),
      mergeMap((action) =>
        this.service.addStudent(action.STUDENTS).pipe(
  ...
  deleteStudentRecord = createEffect(() =>
    this.actions$.pipe(
      ofType(deleteStudent),
      mergeMap((action) =>
        this.service.dletestudent(action.STUDENTID).pipe(
  ...
  getSingleStudent = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap((action) =>
        this.service
          .getSingleStudent(action.STUDENTID)
